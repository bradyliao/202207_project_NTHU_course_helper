...
def course_curriculum_processer(data_folder_path, global_semester):
    ...
    for row in range(0, course_corriculum_df.shape[0]):
        if (pd.isna(course_corriculum_df.iloc[row,1]) ):
            continue
        else:
    ...
    course_corriculum_processed_df.to_csv(data_folder_path + 'log/' + datetime.datetime.now().strftime("%Y%m%d_%H%M%S_") + global_semester + '_course_corriculum_processed.csv', index=False)
